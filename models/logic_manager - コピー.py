...
class LogicManager:
    """
    Handles business logic for Glancal Journey v4.0.
    Implements Strict 3-Mode Logic.
    """
    ...
    def get_current_mode(self) -> Dict[str, any]:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                # Has dispatched
                dep_time_str = row[0]
                # Parse departure time (HH:MM or HH:MM:SS)
                try:
    ...
    def get_items_for_today(self) -> List[dict]:
        """Returns items for today (or empty list if no schedule)."""
        today_str = date.today().isoformat()
    ...
    def get_time_restriction(self) -> Dict[str, any]:
        """Returns time restriction settings for today."""
        today_str = date.today().isoformat()
        schedule = self.db.get_daily_schedule(today_str)
            ...
            if schedule.get("item_ids"):
                ...
                if item_ids:
                    ...
                    with self.db.get_connection() as conn:
                        ...
                        # Note: This list might not preserve order unless we query carefully, but acceptable for now
    ...
    def get_monthly_history(self, year: int, month: int) -> Dict[int, Dict[str, any]]:
        ...
        try:
            with self.db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT date, status, departure_time, points FROM history WHERE date LIKE ?", (month_pattern,))
                rows = cursor.fetchall()
                for row in rows:
                    d_str = row[0]
                    day_int = int(d_str.split("-")[2])
                    history_data[day_int] = {"status": row[1], "time": row[2], "points": row[3]}
        except Exception:
            pass
        return history_data
